...
const requestSchema = z.object({
  ...
  iconPrompt: z.string().trim().min(1).max(4000).optional().nullable(),
});

export default async function handler(req: NextApiRequest, res: NextApiResponse<IconResponse | ErrorResponse>) {
  if (req.method !== "POST") {
  ...
  try {
    ...
    const icon = await generateAndUploadSpellIcon(input, {
      promptOverride: input.iconPrompt ?? null,
    });
